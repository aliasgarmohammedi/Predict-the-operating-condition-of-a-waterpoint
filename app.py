...
def table():
    ...
    data = data.head(50)
    return flask.render_template('index.html', tables=[data.to_html()], titles=[''])



@app.route('/Functional_status_prediction', methods=['POST'])
def Functional_status_prediction():
    start = timer()

    to_predict_row_number = request.form.to_dict()
    to_predict_row_number = list(to_predict_row_number.values())
    to_predict_row_number = int(to_predict_row_number[0])
    ...
    input_to = test_list[to_predict_row_number]
    
    training_df.loc[len(training_df)] = input_to
    ...
    loaded_best_model = pickle.load(open(best_model_filename_rf, 'rb'))
    ...
    if predicted_label[0] == 0:
      prediction='Pump is functional.'
    elif predicted_label[0] == 1:
      prediction='Pump is functional but needs repair.'
    else:
      prediction='Pump is non-functional.'

    end = timer()
    time_elapsed=end-start

    return jsonify([{'prediction': prediction},
                    {'Test Data Point':input_to},
